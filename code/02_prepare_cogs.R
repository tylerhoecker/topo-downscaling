...
lf_water_mask <- rast('../data/lf_water_mask_32611.tif')

# Resample water mask to an example of topo and terra so it matches each
grid_topo <- rast('../data/climate_inputs/tmin_topo_1981-2010.tif')
...
files |> 
  map(function(file){
    ...
    if(dataset == 'topo'){
      grid_final <- mask(grid_utm, lf_water_mask_topo)
    }else{
      grid_final <- crop(grid_utm, grid_topo_utm)
    }
