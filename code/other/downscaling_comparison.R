...
topoclim <- rast('data/topofire/def_1981-2010.tif')
names(topoclim) <- 'topo_val'
...
topo_sample <- terra::extract(topoclim, sample_pts)
...
tile_sample <- terra::extract(out_stack, sample_pts) |> 
  ...
  left_join(topo_sample, by = 'ID')

# Calculate fit
fit_fn <- function(data){
  # Fit the linear model
  lm_mod <- lm(topo_val ~ pred_val, data = data)
...
ggplot(slice_sample(tile_sample, n = 5000)) +
  geom_point(aes(x = pred_val, y = topo_val), alpha = 0.5) +
  ...
  geom_label(data = model_df,
             aes(x = 1700, y = 800, label = r2)) + #x = 100, y = 450  x = 1700, y = 800
  facet_grid(buff~method) +
  theme_bw(base_size = 14) +
  labs(x = 'Predicted 250-m value', y = 'TopoClimate at 250-m centroid', #'TerraClimate 4-km at fine centroid'
       title = tile)

ggsave(paste0('fits_TOPO',tile,'.png'), width = 8, height = 6, dpi = 300)
