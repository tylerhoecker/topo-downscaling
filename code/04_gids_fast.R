...
#           
#           # Stack of coarse files (all variables) for this time --
#           coarse_files <- paste0('../data/cogs/',clim_vars,'_',coarse_name,'_',time_period,'.tif')
#           ds_coarse <- rast(coarse_files)
#           names(ds_coarse) <- clim_vars
#           
#           # Coarse extract for all variables, within buffer distance, returns list for each buffer
...
#             cbind(result_df_wide) |> 
#             rast(type = 'xyz', crs = tile_rast) %>% 
#             writeRaster(paste0('../data/gids_output/ds-220-m_',clim_vars,'_',time,tile),
#                         overwrite = T)
#           
#           rm(focal_result_df, result_df, result_df_wide, tile_rast, 
#              fine_centroids, fine_buffers, coarse_dat, template_dat, fine_dat)
